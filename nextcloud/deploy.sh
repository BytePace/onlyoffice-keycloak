...
for c in docker curl openssl jq; do command -v "$c" >/dev/null 2>&1 || fail "Missing $c"; done
command -v docker-compose >/dev/null 2>&1 || docker compose version >/dev/null 2>&1 || fail "docker compose not found"
...
if [[ "$SETUP_NGINX" == true ]]; then
  ...
  APP_DOMAIN="$APP_DOMAIN" CERTBOT_EMAIL="$CERTBOT_EMAIL" NC_PORT="$NC_PORT" OO_PORT="$OO_PORT" bash "$SCRIPT_DIR/scripts/setup-nginx.sh"
fi
